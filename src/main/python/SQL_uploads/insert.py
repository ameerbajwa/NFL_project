    ...
    for i in range(0,len(passing_stats_df)):
        ...
        with connection_to_database.cursor() as cursor:
            cursor.execute(insert_SQL_query, (
                ...
                int(passing_stats_df.loc[i, 'Cmp']),
                int(passing_stats_df.loc[i, 'Att']),
                int(passing_stats_df.loc[i, 'Yds']),
                int(passing_stats_df.loc[i, '1D']),
                float(passing_stats_df.loc[i, '1D%']),
                int(passing_stats_df.loc[i, 'IAY']),
                float(passing_stats_df.loc[i, 'IAY/PA']),
                int(passing_stats_df.loc[i, 'CAY']),
                float(passing_stats_df.loc[i, 'CAY/Cmp']),
                float(passing_stats_df.loc[i, 'CAY/PA']),
                int(passing_stats_df.loc[i, 'YAC']),
                float(passing_stats_df.loc[i, 'YAC/Cmp']),
                int(passing_stats_df.loc[i, 'Drops']),
                float(passing_stats_df.loc[i, 'Drop%']),
                int(passing_stats_df.loc[i, 'BadTh']),
                float(passing_stats_df.loc[i, 'Bad%']),
                int(passing_stats_df.loc[i, 'Bltz']),
                int(passing_stats_df.loc[i, 'Hrry']),
                int(passing_stats_df.loc[i, 'Hits']),
                int(passing_stats_df.loc[i, 'Scrm']),
                float(passing_stats_df.loc[i, 'Yds/Scr']),
                int(passing_stats_df.loc[i, 'Passing_TD']),
                int(passing_stats_df.loc[i, 'Passing_Int']),
                float(passing_stats_df.loc[i, 'Sk']),
                int(passing_stats_df.loc[i, 'Passing_sacked_Yds']),
                int(passing_stats_df.loc[i, 'Passing_Lng']),
                float(passing_stats_df.loc[i, 'Passing_Rate']),
                int(passing_stats_df.loc[i, 'Fumbles_Fmb']),
                int(passing_stats_df.loc[i, 'Fumbles_FL'])
            ))
